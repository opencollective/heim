...
impl Process {
    ...
    pub fn exe(&self) -> impl Future<Output = ProcessResult<PathBuf>> {
        ...
        future::lazy(move |_| {
            ...
            if pid == 0 || pid == 4 {
                ...
                let handle = bindings::ProcessHandle::query_limited_info(pid).map_err(|e| {
                    match e.kind() {
    ...
    pub fn suspend(&self) -> impl Future<Output = ProcessResult<()>> {
        ...
        if self.pid == 0 {
            ...
            let f = future::lazy(move |_| {
                let handle = bindings::ProcessHandle::for_suspend_resume(pid).map_err(|e| {
                    match e.kind() {
                        io::ErrorKind::PermissionDenied => ProcessError::AccessDenied(pid),
                        _ => e.into(),
                    }
    ...
    pub fn kill(&self) -> impl Future<Output = ProcessResult<()>> {
        ...
        if self.pid == 0 {
            ...
            let f = future::lazy(move |_| {
                let handle = bindings::ProcessHandle::for_termination(pid).map_err(|e| {
                    match e.kind() {
                        io::ErrorKind::PermissionDenied => ProcessError::AccessDenied(pid),
                        _ => e.into(),
                    }
...
fn get_unchecked(pid: Pid) -> impl Future<Output = ProcessResult<Process>> {
    self::create_time::get(pid).map_ok(move |create_time| {
        Process {
            pid,
