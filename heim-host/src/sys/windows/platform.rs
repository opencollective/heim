...
pub async fn platform() -> Result<Platform> {
    // Doing early exit in case of the trouble
    let hostname = wrappers::get_computer_name()?;
    let version = wrappers::rtl_get_version()?;

    Ok(Platform {
        sysinfo: wrappers::get_native_system_info(),
        version,
        hostname,
